...
for phase in range(1,9):
	...
	count=0
	for i in range(0,lon4num-lon3num+1):
		for j in range(0,widenum):
			umean+=u[:,:,lat4num+j+i,lon3num+i-j]
			vmean+=v[:,:,lat4num+j+i,lon3num+i-j]
			count+=1
	umean=umean/count
	vmean=vmean/count
	print count
	...
	#plotwind=umean*np.sqrt(2)/2.+vmean*np.sqrt(2.)/2.
	#brwest
	plotwind=umean*np.sqrt(2)/2.-vmean*np.sqrt(2.)/2.
	...
	plt.title("Phase%02d"%phase)
	plt.savefig("figure/wh04_"+dire+"/wind/wind_p%02d"%phase)
	#plt.show()
	plt.clf()			
