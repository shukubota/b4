...
for i in range(1,9):
    ...
    m.contourf(x, y, plotcp,np.arange(0,1.3,0.1),cmap=cm.Blues)
    #m.contourf(x, y, plotcp)
    #m.contourf(x, y, plotcp,np.arange(0,25,1))
    m.colorbar()
    output="figure/bimodal/daily"+"%02d"%i+".png"
...
os.system("convert -delay 80 -loop 0 figure/bimodal/*.png bimodal_daily.gif")
