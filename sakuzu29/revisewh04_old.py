...
for i in range(1,9):
	...
	for j in range(0,24):
		...
		hour="%02d"%j
		print hour
		data=np.loadtxt("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu29/plotdata_2013/"+file+"/"+hour+".dat")
		countdata=np.loadtxt("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu29/plotdata_2013/"+file+"/count"+hour+".dat")
		...
		for k in range(mstart,mend+1):
			for l in range(nstart,nend+1):	
				...
				datarv[k,l]=(data[k-lag:k+lag+1,l-lag:l+lag+1]*countdata[k-lag:k+lag+1,l-lag:l+lag+1]).sum()
				if countdata[k-lag:k+lag+1,l-lag:l+lag+1].sum()!=0.:
					datarv[k,l]=datarv[k,l]/countdata[k-lag:k+lag+1,l-lag:l+lag+1].sum()
